...
// lou's list archival data of course registration
// https://rabi.phys.virginia.edu/mySIS/CS2/enrollmentData.php?Semester=1192&ClassNumber=10614
...
var getCreateUpdate = function(path, data){
  ...
  for (var i=0; i<subjects.length; i++){
    ...
    for (var j=0; j<subject.career.length; j++) {
      ...
      await scraper.getCourses(career, subject.subjectIndex, subject.id).then( c=>{
          ...
          return Promise.all(
            c.map((course)=>{
              return getCreateUpdate('course/'+course.id, course);
            })
          );
        })
...
*/
  ...
  for (var i=0; i<courses.length; i++) {
      ...
      await scraper.getSections(null,course.id, course.career, course.subjectIndex, course.subject).then( c=>{
        ...
        return Promise.all(
          c.map((section)=>{
            return getCreateUpdate('section/'+section.id, section);
          })
        );
      })
  }
